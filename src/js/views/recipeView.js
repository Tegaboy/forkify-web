...
class ReceipeView extends Views {
  ...
  _errorMessage = 'Could not get receipe <br> Please try another one';
  ...
  addEventHandler(control) {
    ['load', 'hashchange'].forEach(ev => window.addEventListener(ev, control));
  }
  ...
  addEventHandlerServings(handler) {
    this._parentElement.addEventListener('click', function (e) {
      ...
      if (newServing >= 1 && newServing <= 20) handler(newServing);
    });
  }
